...
Deckdle._displayGameSolution = function () {
  let html = ''
  ...
  Object.keys(Deckdle.__getConfig('daily').solutionSet)
    .reverse()
    .forEach((key) => {
      if (key != 10) {
        if (
          Object.keys(Deckdle.__getConfig(gameMode).solutionSet).includes(key)
        ) {
          var solutionWords = []

          // create sorted array of each length category's words
          var sortedArr = Array.from(
            Object.keys(Deckdle.__getConfig(gameMode).solutionSet[key])
          ).sort()
          ...
          sortedArr.forEach((word) => {
            // mark guessed words
            if (Deckdle.__getState(gameMode).guessedWords.includes(word)) {
...
Deckdle._winGameHax = async function (state = null) {
  ...
  try {
    ...
    if (confirmed) {
      const solutionSet = Deckdle.__getConfig().solutionSet
      const solutionSetSize = Deckdle.__getSolutionSize()

      Deckdle.modalOpen('win-game-hax')
      ...
      if (state == 'almost') {
        ...
        Object.keys(solutionSet).forEach((category) => {
          ...
          Object.keys(solutionSet[category]).forEach((word) => {
            if (solutionSet[category][word] == 0) {
              Deckdle.__getConfig().solutionSet[category][word] = 1
        ...
        Deckdle._setScore(Deckdle.__getSolutionSize())
